$(function(){
    ...
    if(splash){
       ...
       if(localStorage.getItem('fdgs' +
       	...
       	console.log(2);
           localStorage.setItem('fdgs','diyici');
            $("html")[0].style.overflow="hidden";
...
// *********************************************************************
	...
	window.addEventListener('resize',function(){
		Hcx = $('body').width();
		hjPageContain.css({
			'-webkit-transform':"translateX("+hjI*-Hcx+"px)",
...
hjSshowItemsSild.on('touchstart',function(e){
	...
	hjSshowItemsSild.on('touchmove',function(e){
		...
		hjSshowItemsSild.css({
			...
			transition:'all 0s',
			'-webkit-transform':"translateX("+runX+"px)",
			transform:"translateX("+runX+"px)"
		});
	});
			...
			if(mx-cx<0){
				if(Math.abs(mx-cx)>100){
					...
					hjSshowItemsSild.css({
						...
						transition:'all 1s',
						'-webkit-transform':"translateX("+Hcx/1.84*-hjNumT+"px)",
